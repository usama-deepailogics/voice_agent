...
from utils.info_extraction import extracting_info, end_call_status
...
async def store_skills_experience(params: Dict) -> Dict:
    ...
    try:
        ...
        candidate_name = params.get("candidate_name", "the candidate")        # Ensure the database file exists
...
FUNCTION_DEFINITIONS = [
    ...
    },
    {
        ...
        "parameters": {
            ...
            "required": ["candidate_name", "position","doc_id"],
        },
    },
]
...
def make_outbound_call(to_number, from_number):
    ...
    <Response>
        ...
        <Connect>
            <Stream url="wss://bce8-137-184-132-246.ngrok-free.app/twilio" />
        </Connect>
    </Response>'''
...
async def initiate_interview_process(candidate_name_payload: CandidatePayload):
    logger.info(f"Initiating interview process for: {candidate_name_payload.doc_id}")
    try:
        # Use the payload candidate_name
        doc_id  = candidate_name_payload.doc_id


        candidate_info = extracting_info(doc_id) #
        ...
        # You might need to re-fetch and re-format PROMPT_TEMPLATE or pass it around
        # PROMPT_TEMPLATE = original_prompt_template.format(...) # Be careful with global modification
